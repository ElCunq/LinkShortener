...
- [x] **AWS ECR Public Mirror Fix**: Replaced standard Docker Hub `node:20-alpine` with `public.ecr.aws/docker/library/node:20-alpine` in `Dockerfile`, bypassing Docker Hub TLS handshake timeouts and rate limits during Coolify automated `--pull --no-cache` builds.
