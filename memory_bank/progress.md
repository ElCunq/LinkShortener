...
- [x] **Atomic Traefik Dynamic Router Generation (Coolify B-Mode)**: Fully implemented `TraefikService` (`src/services/traefikService.ts`) to write `/data/coolify/proxy/dynamic/shortlink-domains.yml` atomically (`.tmp` -> `rename`). Coolify's Traefik detects dynamic rules instantly, issues Let's Encrypt certificates automatically, and eliminates "no available server" errors without manual UI domain entries or Docker redeploys.
